...
import { buildRealHamoshavaData, GATE_POSITIONS } from '../data/stadiumData';
import type { RealSection, RealSeat } from '../data/stadiumData';

const MIN_ZOOM = 0.3;
const MAX_ZOOM = 8;

/** Inner Y of north/south stands in Vivenu SVG space (paths unchanged). Small margin = thin neutral rim. */
const VIVENU_STAND_INNER_NORTH = -1264;
const VIVENU_STAND_INNER_SOUTH = 1183;
const PITCH_EDGE_MARGIN = 12;
const PITCH_X = -1100;
const PITCH_W = 2200;
/** Original decorative pitch height for scaling penalty markings. */
const REF_PITCH_TOP = -600;
const REF_PITCH_H = 1200;

/** Section bbox should fit within this fraction of the map viewport (≈75% of screen). */
...
/** Row merge: one logical row even with a visual gap along the row (see Bloomfield). */
...
export default function HaMoshavaMap() {
  const locale = useLocale();
  const data = useMemo(() => buildRealHamoshavaData(), []);
  ...
  const selectedSectionSeats = useMemo(() => {
    ...
    return data.seats.filter(
      (seat) =>
        seat.fill === sec.fill &&
        Math.abs(seat.cx - sec.centroidX) < 1000 &&
        Math.abs(seat.cy - sec.centroidY) < 1000,
    );
  ...
  const [vbX, vbY, vbW, vbH] = data.viewBox.split(/[\s,]+/).map(Number);

  const pitch = useMemo(() => {
    const top = VIVENU_STAND_INNER_NORTH + PITCH_EDGE_MARGIN;
    const bottom = VIVENU_STAND_INNER_SOUTH - PITCH_EDGE_MARGIN;
    const h = bottom - top;
    const scale = h / REF_PITCH_H;
    const cy = (top + bottom) / 2;
    const mapY = (yRef: number) => top + (yRef - REF_PITCH_TOP) * scale;
    const mapH = (hRef: number) => hRef * scale;
    const centerCircleR = Math.min(290, Math.round(150 * scale));
    return { top, bottom, h, scale, cy, mapY, mapH, centerCircleR };
  }, []);
    ...
    <>
      <div className="flex flex-col w-full min-h-0 h-[100dvh] bg-[#eef1f6] text-slate-800 overflow-hidden">
        <div className="flex flex-shrink-0 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 py-2.5 shadow-sm backdrop-blur-sm">
          <h1 className="text-base font-semibold tracking-tight text-slate-800">HaMoshava Stadium</h1>
        ...
        >
          ...
          >
            ...
            >
              <defs>
                <linearGradient id="hamoshava-grass" x1="0" y1="0" x2="0" y2="1">
              ...
              <rect x={PITCH_X} y={pitch.top} width={PITCH_W} height={pitch.h} rx={20}
                fill="url(#hamoshava-grass)" stroke="#3d7a46" strokeWidth={4} />
              <rect x={PITCH_X} y={pitch.top} width={PITCH_W} height={pitch.h} rx={20}
                fill="none" stroke="#e8f5e9" strokeWidth={2} strokeOpacity={0.9} />
              <line x1={0} y1={pitch.top} x2={0} y2={pitch.bottom} stroke="#e8f5e9" strokeWidth={1.5} strokeOpacity={0.9} />
              <circle cx={0} cy={pitch.cy} r={pitch.centerCircleR} fill="none" stroke="#e8f5e9" strokeWidth={1.5} strokeOpacity={0.9} />
              <circle cx={0} cy={pitch.cy} r={4} fill="#e8f5e9" fillOpacity={0.95} />
              <rect x={PITCH_X} y={pitch.mapY(-300)} width={250} height={pitch.mapH(600)} fill="none" stroke="#e8f5e9" strokeWidth={1} strokeOpacity={0.85} />
              <rect x={850} y={pitch.mapY(-300)} width={250} height={pitch.mapH(600)} fill="none" stroke="#e8f5e9" strokeWidth={1} strokeOpacity={0.85} />
              <rect x={PITCH_X} y={pitch.mapY(-150)} width={100} height={pitch.mapH(300)} fill="none" stroke="#e8f5e9" strokeWidth={1} strokeOpacity={0.85} />
              <rect x={1000} y={pitch.mapY(-150)} width={100} height={pitch.mapH(300)} fill="none" stroke="#e8f5e9" strokeWidth={1} strokeOpacity={0.85} />
              ...
              {data.sections.map((section) => {
                ...
                return (
                  <path
                    ...
                    stroke={strokeColor}
                    strokeWidth={isSelected ? 8 : isHovered ? 6 : 2}
              ...
              {zoom >= 0.6 &&
                data.sections.map((section) => {
                  ...
                  const fontSize = Math.max(30, Math.min(120, sizeScale * 60));
                  return (
                    <text
              ...
              {showSeats &&
                selectedSectionSeats.map((seat, i) => {
                  ...
                  return (
                    <g key={i}>
                      {isSel && (
                        <circle cx={seat.cx} cy={seat.cy} r={14} fill="#0d9488" fillOpacity={0.22} style={{ pointerEvents: 'none' }} />
                      )}
                      <circle
                        data-seat-dot
                        cx={seat.cx}
              ...
              {GATE_POSITIONS.map(({ gate, x, y, label }) => (
                <g key={gate}>
                  <rect x={x - 120} y={y - 40} width={240} height={80} rx={18} fill="#475569" fillOpacity={0.92} />
                  <text
                    x={x}
                    y={y + 10}
                    textAnchor="middle"
      ...
      {panelSection && (
        <aside
          ...
          onMouseDown={(e) => e.stopPropagation()}
          role="complementary"
          aria-label={isHe ? 'סל בחירה — המושבה' : 'Selection — HaMoshava'}
        >
